...
func generateHairLines0(faceContour []Point, numHairLines int) [][]Point {
	...
	for i := 0; i < numHairLines; i++ {
		numHairPoints := 20 + int(math.Floor(randomInRange(-5, 5)))
		hairLine := make([]Point, 0)
		indexOffset := int(math.Floor(randomInRange(30, 140)))
...
func generateHairLines1(faceContour []Point, numHairLines int) [][]Point {
	...
	for i := 0; i < numHairLines; i++ {
		numHairPoints := 20 + int(math.Floor(randomInRange(-5, 5)))
		hairLine := make([]Point, 0)
		indexStart := int(math.Floor(randomInRange(20, 160)))
...
func generateHairLines2(faceContour []Point, numHairLines int) [][]Point {
	...
	for i := range pickedIndices {
		pickedIndices[i] = int(math.Floor(randomInRange(10, 180)))
	}
	sort.Ints(pickedIndices)
	for _, indexOffset := range pickedIndices {
		numHairPoints := 20 + int(math.Floor(randomInRange(-5, 5)))
		hairLine := make([]Point, 0)
		lower := randomInRange(0.8, 1.4)
		reverse := 1
		if rand.Float64() > 0.5 {
			reverse = -1
		}
		for j := 0; j < numHairPoints; j++ {
			powerscale := randomInRange(0.1, 3)
...
func generateHairLines3(faceContour []Point, numHairLines int) [][]Point {
	...
	sort.Ints(pickedIndices)
	splitPoint := int(math.Floor(randomInRange(0, 200)))
	for _, indexOffset := range pickedIndices {
		numHairPoints := 30 + int(math.Floor(randomInRange(-8, 8)))
		hairLine := make([]Point, 0)
		lower := randomInRange(1, 2.3)
		if rand.Float64() > 0.9 {
			lower = randomInRange(0, 1.0)
		}
		reverse := 1
